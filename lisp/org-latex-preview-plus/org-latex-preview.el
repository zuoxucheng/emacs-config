...
(declare-function org-async-wait-for "org-latex-preview-plus-compat")
(declare-function org-latex--precompile "org-latex-preview-plus-compat")
(declare-function org-latex--remove-cached-preamble "org-latex-preview-plus-compat")

(defvar org-latex-precompile)
(defvar org-latex-preview-interfaces)
(defvar org-latex-preview-live)
(defvar-local org-latex-preview--preamble-content nil
  "Cache of the LaTeX preamble for snippet preview.")

;; Generic interface for org-latex-preview
;; ----------------------------------------------------------------
(defvar-local org-latex-preview--fcontext nil
  "Function to return the element at point")
(defvar-local org-latex-preview--ftype nil
  "Function to return the element type of an element.")
(defvar-local org-latex-preview--fproperty nil
  "Function to return the properties `:value', `:begin', `:end',
`:buffer', `:post-blank' and `:post-affiliated'.")
(defvar-local org-latex-preview--fget-numbered nil
  ";TODO: org-latex-preview--get-numbered-equations")
(defvar-local org-latex-preview--fcount-numbered nil
  ";TODO: org-latex-preview--count-numbered-equations")
(defvar-local org-latex-preview--fpreamble nil
  ";TODO: org-latex-preview--get-preamble")
(defvar-local org-latex-preview--fsection nil
  ";TODO: org-latex-preview--section-bounds")

(defun org-latex-preview--setup-interface ()
  ";TODO: "
  ;; TODO: Find the interface functions based on `derived-mode-p',
  ;; with inheritance.
  (if-let* ((interface (alist-get major-mode org-latex-preview-interfaces
                                 nil nil
                                 (lambda (listed current)
                                   (provided-mode-derived-p
                                    current listed)))))
      (setq
       org-latex-preview--fpreamble (plist-get interface :preamble)
       org-latex-preview--fsection (plist-get interface :section)
       org-latex-preview--fcontext (plist-get interface :context)
       org-latex-preview--ftype (plist-get interface :type)
       org-latex-preview--fproperty (plist-get interface :property)
       org-latex-preview--fcount-numbered (plist-get interface :count-numbered)
       org-latex-preview--fget-numbered (plist-get interface :list-numbered))
    (setq org-latex-preview--fpreamble nil
          org-latex-preview--fsection nil
          org-latex-preview--fcontext nil
          org-latex-preview--ftype nil
          org-latex-preview--fproperty nil
          org-latex-preview--fcount-numbered nil
          org-latex-preview--fget-numbered nil)
    (user-error "No interface available for LaTeX previews.")))

;; ;; The defstruct design would be too verbose, see below
;; (cl-defstruct org-latex-preview
;;   "Generic interface functions for using `org-latex-preview' in any
;; major mode."
;;   context type property numbered numbered-count
;;   )

;; (org-element-type (org-element-context))

;; (funcall (org-latex-preview-type org-latex-preview-interfaces)
;;          (funcall (org-latex-preview-context org-latex-preview-interfaces)))

;; (funcall org-latex-preview--ftype (funcall org-latex-preview--fcontext))

(defcustom org-latex-preview-interfaces
  `((org-mode
     :preamble ,#'org-latex-preview--get-preamble
     :section  ,#'org-latex-preview--section-bounds
     :context  ,#'org-element-context
     :type     ,#'org-element-type
     :property ,#'org-element-property
     :list-numbered ,#'org-latex-preview--get-numbered-environments
     :count-numbered ,#'org-latex-preview--count-numbered-equations))
  "Alist of generic functions for `org-latex-preview' operations.
The car of each element is a major mode symbol, and the cdr is a
plist with the following keys, each of which should be a
function:

preamble  : returns a LaTeX preamble
section   : returns section boundaries for point.
context   : returns an element context.
type      : returns the type of an element.
property  : returns the properties `:value', `:begin', `:end',
            `:buffer', `:post-blank' and `:post-affiliated'.
           (The last two are optional.)
numbered  : returns a list of numbered elements.
numbered-count : returns the count of numbered equations in a
                 LaTeX environment."
...
If an org-latex-overlay is already present, nothing is done."
  (and (not (eq (get-char-property (point) 'org-overlay-type)
                'org-latex-overlay))
       (when-let* ((element (funcall org-latex-preview--fcontext))
                   ((eq (funcall org-latex-preview--ftype element) type))
                   (elem-beg (or (funcall org-latex-preview--fproperty :post-affiliated element)
                                 (funcall org-latex-preview--fproperty :begin element)))
                   (elem-end (- (funcall org-latex-preview--fproperty :end element)
                                (or (funcall org-latex-preview--fproperty :post-blank element) 0)
                                (if (eq (char-before (funcall org-latex-preview--fproperty :end element))
                                        ?\n)
                                    1 0)))
  ...
  (with-current-buffer (overlay-buffer ov)
    (let* ((start (overlay-start ov))
           ...
           (others (and org-latex-preview-numbered
                        ...
                        (eq (funcall org-latex-preview--ftype fragment) 'latex-environment)
                        (funcall org-latex-preview--fget-numbered end nil))))
      (if (memq (funcall org-latex-preview--ftype fragment)
                '(latex-fragment latex-environment))
          (if (org-latex-preview--empty-fragment-p
               (funcall org-latex-preview--fproperty :value fragment))
...
customize the variable `org-latex-preview-live'."
  ...
  (if org-latex-preview-auto-mode
      (progn
        (org-latex-preview--setup-interface)
    ...
    (let ((latex-env-p
           (progn
             (unless org-latex-preview-live--element-type
               (let* ((elm (funcall org-latex-preview--fcontext)))
                 ...
                 (setq org-latex-preview-live--element-type
                       (or (and (string-prefix-p
                                 "\\[" (funcall org-latex-preview--fproperty :value elm))
                                'latex-environment)
                           (funcall org-latex-preview--ftype elm)))))
  ...
  (when (and (equal major-mode (org-src-get-lang-mode "latex"))
    ...
    (let* ((org-buf (marker-buffer org-src--beg-marker))
      ...
      (with-current-buffer org-buf
        (setq element (funcall org-latex-preview--fcontext))
        (pcase (funcall org-latex-preview--ftype element)
          ('export-block (setq skip-env-p t))
          ('latex-environment
           (setq skip-env-p
                 (and (save-excursion
                        (goto-char (or (funcall org-latex-preview--fproperty :post-affiliated element)
                                       (funcall org-latex-preview--fproperty :begin element)))
                        (looking-at "\\\\begin{\\([^}]+\\)}"))
          ...
          (setq ov (copy-overlay orig-ov)
                preamble (or org-latex-preview--preamble-content
                             (setq org-latex-preview--preamble-content
                                   (funcall org-latex-preview--fpreamble))))
          (overlay-put ov 'view-text t)
      ...
      (unless skip-env-p
        ...
        (when (and org-latex-preview-numbered
                   (eq (funcall org-latex-preview--ftype element) 'latex-environment))
          (with-current-buffer org-buf
        ...
        (when (buffer-local-value 'org-latex-preview-live org-buf)
          (if org-buf-visible-p
            ...
            (let* ((element-type
                    (with-current-buffer org-buf
                      (or (and (string-prefix-p
                                "\\[" (funcall org-latex-preview--fproperty :value element))
                               'latex-environment)
                          (funcall org-latex-preview--ftype element))))
...
will be treated as \"point\"."
  ...
  (when (display-graphic-p)
    (unless org-latex-preview--ftype (org-latex-preview--setup-interface))
    (unless (and mode (symbolp mode))
      (setq mode
            (pcase (car-safe mode)
              ...
              (_ (if-let* ((datum (funcall org-latex-preview--fcontext)) ; Auto, i.e. element at point or section
                          ((memq (funcall org-latex-preview--ftype datum)
    ...
    (pcase mode
      ...
      ('section
       (apply #'org-latex-preview--preview-region
              org-latex-preview-process-default
              (funcall org-latex-preview--fsection)))
      ('clear-section
       (apply #'org-latex-preview-clear-overlays
        (funcall org-latex-preview--fsection)))
      ('region
       (org-latex-preview--preview-region
      ...
      ('point
       (when-let* ((datum (funcall org-latex-preview--fcontext))
                  ((memq (funcall org-latex-preview--ftype datum) '(latex-environment latex-fragment))))
         (org-latex-preview--auto-aware-toggle datum)))
      ((guard (memq (funcall org-latex-preview--ftype mode) '(latex-environment latex-fragment)))
...
should it be enabled."
  (let* ((beg (funcall org-latex-preview--fproperty :begin datum))
         (end (funcall org-latex-preview--fproperty :end datum))
         (ov (cl-some
              (lambda (o)
  ...
  (let (fragments)
    (save-excursion
      ...
      (while (re-search-forward org-latex-preview--tentative-math-re end t)
        (let ((obj (funcall org-latex-preview--fcontext)))
          (when (and (memq (funcall org-latex-preview--ftype obj)
                           '(latex-fragment latex-environment))
                     ;; Avoid duplicating nested latex environments
                     (not (and fragments
                               (= (funcall org-latex-preview--fproperty :begin obj)
                                  (funcall org-latex-preview--fproperty :begin (car fragments))))))
            (push obj fragments)))))
    (nreverse fragments)))
...
the number offsets will also be calculated, using PARSE-TREE if given."
  (let ((numbering-table (and construct-numbering-p
    ...
    (dolist (element elements)
      (let ((beg (or (funcall org-latex-preview--fproperty :post-affiliated element)
                     (funcall org-latex-preview--fproperty :begin element)))
            (end (- (funcall org-latex-preview--fproperty :end element)
                    (or (funcall org-latex-preview--fproperty :post-blank element) 0)
                    (if (eq (char-before (funcall org-latex-preview--fproperty :end element))
                            ?\n)
                        1 0)))
            (content (funcall org-latex-preview--fproperty :value element)))
        (push (list beg end content) entries)
        (when numbering-table
          (push (and (eq (funcall org-latex-preview--ftype element) 'latex-environment)
...
OPTIONS is a plist containing foreground/background specifications."
  (let* ((face (org-latex-preview--face-around start end))
         ...
         (bg (pcase (plist-get options :background)
               ...
               (color color))))
    (list (if (member fg '(unspecified unspecified-fg
                           "unspecified" "unspecified-fg"))
              "Black"
            fg)
          (if (member bg '(unspecified unspecified-bg
...
`org-latex-preview--get-numbered-environments'."
  (let ((table (make-hash-table
        ...
        (counter 1))
    (save-match-data
      (dolist (element (funcall org-latex-preview--fget-numbered
                        nil nil parse-tree))
        (puthash element counter table)
        (cl-incf counter (funcall org-latex-preview--fcount-numbered element))))
    table))
...
Note: this function changes the current match data."
  (or (org-element-cache-get-key element :latex-numbered-equation-count)
      (let ((count 0)
            environment-name)
        (with-current-buffer (funcall org-latex-preview--fproperty :buffer element)
          (save-excursion
            (goto-char (funcall org-latex-preview--fproperty :begin element))
            (when (looking-at "\\\\begin{\\([^}]+\\)}")
              (setq environment-name (match-string 1)))
            (cond
             ((member environment-name org-latex-preview--numbered-environments-single)
              (unless (or (search-forward "\\nonumber" (funcall org-latex-preview--fproperty :end element) t)
                          (search-forward "\\tag{" (funcall org-latex-preview--fproperty :end element) t))
                (cl-incf count 1)))
...
the *entire* preview cache will be cleared, and `org-persist-gc' run."
  (interactive
   (if current-prefix-arg
     ...
     (let ((context (funcall org-latex-preview--fcontext)))
       (cond
        ((use-region-p)
         (list (region-beginning) (region-end)))
        ((memq (funcall org-latex-preview--ftype context)
               '(latex-fragment latex-environment))
         (list (funcall org-latex-preview--fproperty :begin context)
               (funcall org-latex-preview--fproperty :end context)))
        (t (list nil nil))))))
  ...
  (if clear-entire-cache
    ...
    (let ((imagetype
      ...
      (dolist (element (org-latex-preview-collect-fragments beg end))
        (pcase-let* ((begin (or (funcall org-latex-preview--fproperty :post-affiliated element)
                                (funcall org-latex-preview--fproperty :begin element)))
                     (end (- (funcall org-latex-preview--fproperty :end element)
                             (or (funcall org-latex-preview--fproperty :post-blank element) 0)
                             (if (eq (char-before (funcall org-latex-preview--fproperty :end element))
                     ...
                     (value (funcall org-latex-preview--fproperty :value element))
                     (number (and numbering-table
                                  (eq (funcall org-latex-preview--ftype element)
                                      'latex-environment)
                                  (gethash element numbering-table))))
          (org-latex-preview--remove-cached
